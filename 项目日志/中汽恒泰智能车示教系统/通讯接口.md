...
此函数接受两个参数
- `name`：字符串，全局回调名
- `callback`：全局回调函数，此函数需要有且仅有一个必须的参数用来接收远程目标发送回来的响应。
...
def global_callback(response):
    print 'global_callback', response


server_communication.response_callback('global_callback', global_callback)
request = {
    'action': 'admin_get_vehicle_current_running_data',
    'info': {
        'vehicle_id': 2
...
e=>end: 结束
op0=>operation: 收到响应包
op1=>operation: 将响应包作为参数传入并调用所有全局回调
op2=>operation: 注册回调
op3=>operation: 返回False
op4=>operation: 返回True
cond=>condition: 是否存在同名回调?
