...
- `student_start_vehicle_system`：开启车载系统
	发送包：
	```py
	{
	# 动作名，必须
...
### 运行状态自检
##### 概述
1. 用户点击运行状态自检按钮，触发`self.ui.button_self_check.clicked`信号，显示自检界面
2. 系统通过[车辆数据服务](http://192.168.10.106:8080/project/3?p=297)取到车辆传感器数据，进行自检。
#####  具体逻辑
...
```
`self.__check`方法中，核心部分在于开启线程调用`self.check_sensors`方法。需要注意的点是，在`start_thread_work(self.check_sensors, args=(vehicle_sensors,))`一句中，`vehicle_sensors`是通过车辆数据服务取到的车辆传感器实时数据。[车辆数据服务](http://192.168.10.106:8080/project/3?p=297)
...
### 运行日志
##### 概述
1. 用户点击运行日志按钮，触发`self.ui.button_view_log.clicked`信号，显示运行日志选择时间界面
2. 用户在确定开始时间与结束时间之后，点击确定按钮，触发`self.commit.clicked`信号，系统会检查：
	1. 起始时间是否小于结束时间
	2. 起始时间与结束时间间隔是否小于1小时
3. 系统访问数据库，取得车辆轨迹数据。
4. 系统对得到的轨迹数据进行坐标变换、偏移调整，并进行轨迹回放。
##### 具体逻辑
...
- `initial_x、initial_y、self.initial_r`：沙盘初始x、y坐标以及角度r偏移。此参数用于含有偏移的二维坐标变换。其在程序中具体体现的代码处于
	...
	```
	其具体原理参考下一节
- `delta_r`：沙盘小车的角度偏移，沙盘中小车的角度0位置是朝向正右方的，随着角度变大，小车会顺时针旋转，`delta_r`用来调整实际角度与沙盘角度之间的偏移，实际程序体现如下：
	```py
	# ui/vehicle_management/sandbox.py	
...
y=y'·cos(θ)-x'·sin(θ)+b
##### 沙盘偏移矫正
由于`QT`中的`Pixmap`的坐标设置是基于边角坐标的，但是我们希望设置的是中心坐标，所以需要对其进行微调，其原理参考上一节二维坐标变换。

``` py
# ui/vehicle_management/sandbox.py
# 沙盘上的像素的坐标
x = self.sand_box_width * pos_x / self.environment_width
x = x - self.vehicle.pixmap().width() * self.sand_box_vehicle_scale / 2
y = self.sand_box_height * pos_y / self.environment_height
y = y - self.vehicle.pixmap().height() * self.sand_box_vehicle_scale / 2
w = self.vehicle.pixmap().width() * self.sand_box_vehicle_scale / 2
h = self.vehicle.pixmap().height() * self.sand_box_vehicle_scale / 2
l = math.sqrt(w * w + h * h)
theta0 = math.atan(h / w)
...
# ui/vehicle_management/sandbox.py
    def replay_running_track_service(self, data):
        ...
        self.vehicle_running_track.append(False)
```
在这个函数中，`self.yield_running_track(data)`产生一个迭代器。这个迭迭代器的主要功能是每迭代一次就向数据库服务端发送一次请求，取得数据，避免一次取大量数据造成延迟。

``` py
# ui/vehicle_management/sandbox_dialog.py
    def yield_running_track(self, data):
        start_time = data[0]
        stop_time = data[1]
        finish = False
        while True:
            ...
            stop_time_str = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(start_time + 9))
            start_time = start_time + 10
            state, result = server_communication.communicate({
                'action': 'admin_get_vehicle_running_data',
                'info': {
            ...
            sim_log(start_time_str)
            sim_log(stop_time_str)
            yield result['info'] if state and result['state'] else []
```
