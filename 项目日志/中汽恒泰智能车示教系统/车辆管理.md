...
```
`self.__check`方法中，核心部分在于开启线程调用`self.check_sensors`方法。需要注意的点是，在`start_thread_work(self.check_sensors, args=(vehicle_sensors,))`一句中，`vehicle_sensors`是通过车辆数据服务取到的车辆传感器实时数据。
