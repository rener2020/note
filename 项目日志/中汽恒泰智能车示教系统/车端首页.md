...
5. 最终，进行下列操作
	...
	4. 访问[上传车辆传感器数据接口](http://192.168.10.106:8080/project/3?p=322)，向服务器上传当前车辆传感器信息
		...
		def start_processing_vehicle_node_data(self):
			...
			... ...
		```
		`log_vehicle_sensor_current_status`函数如下：
		```py
		# ui/VehicleUi.py
		def log_vehicle_sensor_current_status(self):
			start_thread_work(lambda: self.server_communication.send({
				'action': 'vehicle_log_sensor_status',
				'info': self.vehicle_sensor_status_data
			}))
		```
	5. 启动rviz界面，展示车辆信息
		```py
		...
		def start_processing_vehicle_node_data(self):
			...
			... ...
		```
		`self.start_rviz_service`函数如下，可见这个函数循环检查`self.is_ros_core_ok`标志位，以检查车辆roscore是否启动，若能够连接到车辆roscore则提起`rviz`界面。
		```py
		# ui/VehicleUi.py
		def start_rviz_service(self):
			while True:
				time.sleep(1)
				if self.is_ros_core_ok:
					self.signal_write_log.emit(sim_log('正在启动rviz...'))
					self.signal_trigger_rviz.emit()
					self.is_rviz_frame_init = True
					break
		```